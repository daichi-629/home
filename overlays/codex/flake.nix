    ...
    in
    {
      packages = forAllSystems (
        ...
        let
          ...
          mkCodex = packageLockFile: pkgs.callPackage ./package.nix { inherit npm-cli packageLockFile; };
          versionedPackages = builtins.listToAttrs (
            builtins.map (version: {
              name = version;
              value = mkCodex ./versions/${"package-lock_" + version + ".json"};
            }) versionNames
          );
          latestPackage = mkCodex ./versions/${"package-lock_" + latestVersion + ".json"};
        in
        {
          codex = latestPackage;
      ...
      overlays.default = _final: prev: {
        codex = self.packages.${prev.stdenv.hostPlatform.system}.codex;
      };
      ...
      apps = forAllSystems (
        ...
        in
        {
          update = {
            type = "app";
            program = "${update}/bin/update-codex-lock";
          };
        }
      );
    };
